...
//@Service
@RestController
@RequestMapping(value = {"/api/branches", "/${spring.application.name}" + "/api/branches", "oauth2_search" + "/api/branches"})
public class RestCrudBranchController implements RestCrudInterface<EntityResponseDTO, String, EntityRequestDTO> {

    @Autowired
    private CrudBranchElasticLService repository;
    private EntityFactory entityFactory = EntityFactory.getInstance();
    private Branch entity;
    static Logger log = Logger.getLogger(RestCrudBranchController.class.getName());
    ...
    private List<EntityResponseDTO> createEntityList(List<Branch> entityList) {
        if (entityList == null) {
            return null;
        }
